...
function convertMs(timerTimeUTC) {
    ...
    // Remaining minutes
    const minutes = addLeadingZero(Math.floor(timerTimeUTC % day % hour / minute));
    // Remaining seconds
    const seconds = addLeadingZero(Math.floor(timerTimeUTC % day % hour % minute / second));
    return {
        days,
        hours,
        minutes,
...
const hendleSelectedDate = (selectedDate)=>{
    ...
    if (Date.now() > selectedDateUTC) (0, _notiflixDefault.default).Notify.warning("Please choose a date in the future");
    else {
